...
function Checkout() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (search.req && search.token) {
        const res = await getRequestByToken({ data: { requestId: search.req, token: search.token } });
        if (res && !res.blocked && res.event && res.ticket_type) {
          setEv({ id: res.event.id, name: res.event.name, currency: res.event.currency });
          setItems([{ tt: { id: res.ticket_type.id, name: res.ticket_type.name, price: res.ticket_type.price, quantity_sold: 0 }, quantity: res.request.quantity ?? 1 }]);
          setBuyer({ name: res.request.buyer_name, email: res.request.buyer_email, phone: res.request.buyer_phone ?? "" });
          return;
        }
        toast.error("This request is not approved or the link is invalid");
      }
      const raw = sessionStorage.getItem("eos_cart");
      if (!raw) { nav({ to: "/e/$slug", params: { slug } }); return; }
      const cart: { eventId: string; items: CartItem[] } = JSON.parse(raw);
      const { data: e } = await supabase.from("events").select("id,name,currency").eq("id", cart.eventId).maybeSingle();
      setEv(e as EventT | null);
      const ids = cart.items.map((i) => i.id);
      const { data: tts } = await supabase.from("ticket_types").select("id,name,price,quantity_sold").in("id", ids);
      const merged = cart.items
        .map((i) => ({ tt: (tts ?? []).find((t) => t.id === i.id) as TT | undefined, quantity: i.quantity }))
  ...
  if (!ev) return <div className="min-h-screen grid place-items-center text-sm text-muted-foreground">Loading…</div>;
  ...
  return (
    <div className="min-h-screen bg-[#fafafa]">
      <div className="mx-auto max-w-3xl px-6 py-12">
        <Link to="/e/$slug" params={{ slug }} className="text-sm text-muted-foreground hover:text-foreground">← Back to event</Link>
        ...
        <div className="mt-8 grid md:grid-cols-[1fr_320px] gap-6">
          <Card className="p-6 border-black/10">
            <form onSubmit={pay} className="space-y-4">
                ...
                <div className="mt-2 rounded-2xl border border-black/10 bg-gradient-to-br from-pink-50 via-white to-sky-50 p-5 space-y-4">
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-lg bg-black text-white grid place-items-center text-xs font-bold tracking-tight">P+</div>
                      <div>
                        <div className="text-sm font-semibold">PayPlus secure checkout</div>
                        <div className="text-[11px] text-muted-foreground">PCI-DSS · 3D Secure · Tokenized</div>
                      </div>
                    </div>
                  ...
                  {method === "card" && (
                    <div className="rounded-xl bg-white border border-black/10 p-4 space-y-3">
                      ...
                      <div>
                        ...
                        >
                          {[1, 2, 3, 6, 12].map((n) => (
                            <option key={n} value={n}>{n === 1 ? "Pay in full" : `${n} × ${ev.currency} ${(grandTotal / n).toFixed(2)}`}</option>
                          ))}
                        </select>
                      </div>
                    </div>
                  ...
                  {method === "applepay" && (
                    <button type="button" className="w-full h-12 rounded-xl bg-black text-white font-medium flex items-center justify-center gap-2 hover:bg-black/90">
                      <span></span> Pay
                    </button>
                  )}
                  {method === "googlepay" && (
                    <button type="button" className="w-full h-12 rounded-xl bg-white border border-black/20 text-black font-medium flex items-center justify-center gap-2 hover:bg-black/5">
                  ...
                  {method === "bit" && (
                    <div className="rounded-xl bg-white border border-black/10 p-4 text-sm space-y-2">
                      ...
                      <p className="text-[11px] text-muted-foreground">You'll be redirected to the Bit app to approve {ev.currency} {grandTotal.toLocaleString()}.</p>
                    </div>
                  )}

                  <div className="rounded-lg bg-white/70 border border-black/5 p-3 space-y-1.5 text-sm">
                    <div className="flex justify-between text-muted-foreground"><span>Subtotal</span><span>{ev.currency} {subtotal.toLocaleString()}</span></div>
                    <div className="flex justify-between text-muted-foreground"><span>Processing fees</span><span>{fees === 0 ? "Included" : `${ev.currency} ${fees.toLocaleString()}`}</span></div>
                    <div className="flex justify-between text-muted-foreground"><span>VAT</span><span>Included</span></div>
                    <div className="flex justify-between font-display text-base pt-1.5 border-t border-black/10"><span>Total due</span><span>{ev.currency} {grandTotal.toLocaleString()}</span></div>
                  </div>

                  <p className="text-[11px] text-muted-foreground leading-relaxed">
                    Live PayPlus integration is coming soon. Your order is processed in sandbox mode and tickets are issued instantly to your email.
                  </p>
                </div>
              </div>
              ...
              <p className="text-[11px] text-center text-muted-foreground">By completing this purchase you agree to the terms of sale and refund policy.</p>
            </form>
          </Card>
          <Card className="p-5 border-black/10 h-fit">
            ...
            <ul className="mt-3 divide-y divide-black/5 text-sm">
              {items.map((it) => (
                <li key={it.tt.id} className="py-2 flex justify-between">
                  <span>{it.quantity} × {it.tt.name}</span>
                  <span>{ev.currency} {(it.quantity * Number(it.tt.price)).toLocaleString()}</span>
                </li>
              ))}
            </ul>
            <div className="mt-3 pt-3 border-t border-black/10 space-y-1 text-sm">
              <div className="flex justify-between text-muted-foreground"><span>Subtotal</span><span>{ev.currency} {subtotal.toLocaleString()}</span></div>
              <div className="flex justify-between text-muted-foreground"><span>Fees</span><span>{fees === 0 ? "Included" : `${ev.currency} ${fees.toLocaleString()}`}</span></div>
              <div className="flex justify-between font-display text-base pt-2 border-t border-black/10"><span>Total</span><span>{ev.currency} {grandTotal.toLocaleString()}</span></div>
            </div>
            <div className="mt-4 pt-4 border-t border-black/10 flex items-center justify-between text-[11px] text-muted-foreground">
              ...
              <span className="flex items-center gap-1.5">
                <span className="h-5 px-1.5 rounded bg-black text-white text-[9px] font-bold grid place-items-center">P+</span>
                PayPlus
              </span>
            </div>
          </Card>
