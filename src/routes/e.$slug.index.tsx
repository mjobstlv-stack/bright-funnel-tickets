...
import {
  ...
  overlayBackground,
  type LogoLayout,
} from "@/components/event/logo-placement";
...
function SafeLogo({
  ...
  if (!ok) return null;
  return (
    <img src={src} alt={alt} className={className} style={style} onError={() => setOk(false)} />
  );
}
...
function PublicEvent() {
  ...
  const total = useMemo(
    () => tickets.reduce((sum, t) => sum + (qty[t.id] ?? 0) * chargeUnit(t), 0),
    [tickets, qty],
  );
  ...
  const urlTpl =
    typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("tpl") : null;
    ...
    <Card className="p-5 border-black/10 shadow-sm bg-white text-foreground">
      ...
      <div className="mt-4 space-y-3">
        {tickets.length === 0 && (
          <p className="text-sm text-muted-foreground">
            {tx("המכירה עוד לא נפתחה.", "Sales not open yet.")}
          </p>
        )}
        {tickets.map((t) => {
          const remaining = t.quantity_total - t.quantity_sold;
                  ...
                  {isRes ? (
                    <div className="mt-1">
                      <div className="text-sm">
                        {ev.currency} {Number(t.deposit_amount ?? 0).toLocaleString()}{" "}
                        <span className="text-xs text-muted-foreground">
                          {tx("פיקדון עכשיו", "deposit now")}
                        </span>
                      </div>
                      <div className="text-xs text-muted-foreground">
                        ...
                        {t.balance_on_site
                          ? tx(
                              " · היתרה משולמת במקום בכרטיס אשראי",
                              " · balance paid on site by card",
                            )
                          : ""}
                      </div>
                    </div>
                    ...
                    <div className="text-sm mt-1">
                      {ev.currency} {Number(t.price).toLocaleString()}
                    </div>
                  )}
                </div>
                {soldOut ? (
                  <span className="text-xs text-muted-foreground">
                    {tx("אזל המלאי", "Sold out")}
                  </span>
                ) : (
      ...
      <div className="mt-5 pt-4 border-t border-black/5 flex items-center justify-between">
        <div className="text-sm text-muted-foreground">
          ...
          {checkoutItems.some((x) => x.tt.kind === "reservation") && (
            <span className="block text-xs">{tx("לחיוב עכשיו", "charged now")}</span>
          )}
        </div>
        <div className="font-display text-lg">
          {ev.currency} {total.toLocaleString()}
        </div>
      </div>
      <Button
        disabled={ev.requires_approval ? false : itemCount === 0}
        onClick={ev.requires_approval ? () => setRequestOpen(true) : checkout}
        className="mt-4 w-full h-11 rounded-full"
      >
        {ev.requires_approval
          ? tx("בקשה להשתתף", "Request to attend")
          : tx("לתשלום ←", "Checkout →")}
      </Button>
      {ev.requires_approval && (
        <p className="text-[11px] text-muted-foreground mt-2 text-center">
          {tx(
            "נדרש אישור מארגן. לא תחויבו עד לאישור.",
            "Approval required. You won't be charged until approved.",
          )}
        </p>
      )}
    </Card>
        ...
        <TemplateRenderer
          ...
          extras={
            <EventExtras
              ...
              lang={he ? "he" : "en"}
              tone={
                (isDarkColor(ev.bg_color) ?? (template === "poster" || template === "immersive"))
                  ? "dark"
                  : "light"
              }
            />
  ...
  return (
    <div className={`text-sm ${cls}`}>
      <div className="flex flex-wrap items-center gap-4">
        {multi ? (
          compact ? (
            <span className="inline-flex items-center gap-1.5">
              ...
              {formatShortDay(openDays[0].date)} {he ? "עד" : "–"}{" "}
              {formatShortDay(openDays[openDays.length - 1].date)}
        ...
        {ev.event_type !== "virtual" && ev.venue_name && (
          <span className="inline-flex items-center gap-1.5">
        ...
        {ev.event_type !== "physical" && (
          <span className="inline-flex items-center gap-1.5">
            <Video className="h-4 w-4" />
            {he ? "אירוע אונליין" : "Online event"}
          </span>
        )}
      </div>
      {!compact && multi && (
  ...
  return (
    <div className="mt-5 rounded-xl border border-black/10 p-4 bg-gradient-to-br from-pink-50 to-sky-50">
      ...
      <p className="text-sm mt-1">
        {he
          ? "מחזיקי כרטיס יקבלו את קישור הכניסה במייל לאחר הרכישה."
          : "Ticket holders receive the join link by email after purchase."}
      </p>
    </div>
  );
}
...
function TemplateRenderer({
  ...
  ev: EventT;
  ticketCard: ReactNode;
  extras: ReactNode;
  he?: boolean;
}) {
  const isMobile = useIsMobile();
  const device: "mobile" | "desktop" = isMobile ? "mobile" : "desktop";
  const bannerSpot = readSpot(
    ev.logo_layout,
    "banner",
    device,
    defaultBannerSpot(!!ev.circle_logo_url),
  );
  const circleStyles = circleLogoStyle(
    readSpot(ev.logo_layout, "circle", device, defaultCircleSpot()),
  );
  if (template === "poster") {
    return (
      <div className="min-h-screen bg-neutral-950 text-white">
        <div className="relative min-h-[70vh] sm:min-h-[80vh] flex items-center justify-center overflow-hidden">
          <div
            ...
            style={
              ev.cover_url
                ? {
                    backgroundImage: `url(${ev.cover_url})`,
          ...
          <div className="relative z-10 text-center px-6 py-20 sm:py-24 max-w-4xl mx-auto">
            {ev.logo_url && (
              <SafeLogo
                src={ev.logo_url}
                alt="Logo"
                className="mx-auto h-24 sm:h-40 w-auto max-w-[60%] object-contain drop-shadow-2xl"
              />
            )}
            {ev.tagline && (
              <p className="mt-6 uppercase tracking-[0.3em] text-[11px] sm:text-xs text-white/85">
                {ev.tagline}
              </p>
            )}
            <h1 className="mt-3 text-4xl sm:text-6xl lg:text-7xl font-display tracking-tight break-words">
              {ev.name}
            </h1>
            <div className="mt-6 flex justify-center">
              <MetaRow ev={ev} tone="light" compact he={he} />
            </div>
          </div>
        </div>
        <div className="mx-auto max-w-5xl px-6 py-12 sm:py-16 grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_360px] gap-10">
          <div className="min-w-0 order-1">
            <JoinBox ev={ev} he={he} />
            {ev.description && (
              <div className="mt-2 max-w-none whitespace-pre-line text-[15px] leading-[1.75] text-white/85">
                {ev.description}
              </div>
            )}
          </div>
          <div className="min-w-0 order-3 lg:col-start-1 lg:row-start-2">{extras}</div>
          <div className="lg:sticky lg:top-6 self-start min-w-0 order-2 lg:col-start-2 lg:row-start-1 lg:row-span-2">
            {ticketCard}
          </div>
        </div>
      </div>
          ...
          >
            ...
            {ev.logo_url && (
              <SafeLogo
                src={ev.logo_url}
                alt="Logo"
                className="absolute inset-0 m-auto h-1/3 max-h-[45%] w-auto max-w-[60%] object-contain drop-shadow-2xl"
              />
            )}
          </div>
          <div className="flex min-w-0 flex-col gap-3 px-5 sm:px-8 lg:px-12 py-5 lg:py-6 lg:min-h-0 lg:gap-2 lg:overflow-hidden">
            {ev.tagline && (
              <p className="uppercase tracking-[0.22em] text-[10px] sm:text-xs text-muted-foreground line-clamp-1">
                {ev.tagline}
              </p>
            )}
            <h1 className="text-2xl sm:text-4xl lg:text-5xl font-display tracking-tight break-words line-clamp-2">
              {ev.name}
            </h1>
            <div className="shrink-0">
              <MetaRow ev={ev} compact he={he} />
            </div>
            {ev.description && (
    ...
    return (
      <div className="min-h-screen bg-white">
        <div className="mx-auto max-w-3xl px-6 pt-16 sm:pt-24 pb-12 text-center">
          ...
          {ev.tagline && (
            <p className="mt-10 uppercase tracking-[0.3em] text-[11px] text-muted-foreground">
              {ev.tagline}
            </p>
          )}
          <h1 className="mt-3 text-4xl sm:text-6xl lg:text-7xl font-display tracking-tight break-words">
        ...
        <div className="mx-auto max-w-3xl px-6 py-12 sm:py-16 grid grid-cols-1 md:grid-cols-[minmax(0,1fr)_320px] gap-10">
          <div className="min-w-0 order-1">
            <JoinBox ev={ev} he={he} />
            {ev.description && (
              <div className="mt-2 max-w-none whitespace-pre-line text-[15px] leading-[1.75]">
                {ev.description}
              </div>
            )}
          </div>
          <div className="min-w-0 order-3 md:col-start-1 md:row-start-2">{extras}</div>
          <div className="md:sticky md:top-6 self-start min-w-0 order-2 md:col-start-2 md:row-start-1 md:row-span-2">
            {ticketCard}
          </div>
        </div>
      </div>
        ...
        >
          ...
          {ev.logo_url && (
            <SafeLogo
              src={ev.logo_url}
              alt="Logo"
              className="absolute top-6 sm:top-8 left-1/2 -translate-x-1/2 h-12 sm:h-20 w-auto max-w-[60%] object-contain drop-shadow-xl"
            />
          )}
          <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-12 lg:p-16 max-w-4xl">
            {ev.tagline && (
              <p className="uppercase tracking-[0.3em] text-[11px] sm:text-xs text-white/85">
                {ev.tagline}
              </p>
            )}
            <h1 className="mt-3 text-4xl sm:text-6xl lg:text-8xl font-display tracking-tight break-words">
              {ev.name}
            </h1>
            <div className="mt-6">
              <MetaRow ev={ev} tone="light" compact he={he} />
    ...
    <div className="min-h-screen bg-white">
      ...
      >
        <div className="min-w-0 order-1">
          ...
          <h1 className="mt-2 text-3xl sm:text-4xl lg:text-5xl font-display tracking-tight break-words">
            {ev.name}
          </h1>
          <div className="mt-4">
            <MetaRow ev={ev} compact he={he} />
          ...
          <JoinBox ev={ev} he={he} />
          {ev.description && (
            <div className="mt-8 max-w-none whitespace-pre-line text-[15px] leading-[1.7]">
              {ev.description}
            </div>
          )}
        </div>
