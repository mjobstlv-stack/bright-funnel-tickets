...
function NewEvent() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : t("יצירת האירוע נכשלה", "Failed to create event"),
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="max-w-3xl">
      <h1 className="text-3xl font-display tracking-tight">
        {t("יצירת אירוע חדש", "Create a new event")}
      </h1>
      <p className="mt-2 text-muted-foreground">
        {t(
          "אפשר לערוך את כל זה אחר כך. כרטיסים מתווספים לאחר יצירת האירוע.",
          "You can edit any of this later. Tickets are added after you create the event.",
        )}
      </p>
        ...
        <form onSubmit={submit} className="grid sm:grid-cols-2 gap-5">
          ...
          <div className="sm:col-span-2">
            ...
            <p className="text-xs text-muted-foreground mt-1" dir="ltr">
              https://bright-funnel-lab.lovable.app/e/{slug || "your-event"}
            </p>
            <p className="text-xs text-muted-foreground mt-1">
              {t(
                "מותר: אותיות לטיניות, מספרים, נקודות, קווים תחתונים ומקפים. רווחים וסלאשים יומרו למקפים.",
                "Allowed: Latin letters, numbers, dots, underscores and dashes. Spaces and slashes become dashes.",
              )}
            </p>
          </div>
          <div className="sm:col-span-2">
          ...
          {eventType !== "physical" && (
            <div className="sm:col-span-2">
              <Label>
                {t("קישור לפגישה אונליין (אפשר להשאיר ריק)", "Online meeting link (optional now)")}
              </Label>
              <Input
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {t(
                  "אם ריק, ניצור קישור Google Meet אוטומטי מדף האירוע.",
                  "Leave empty to auto-generate a Google Meet link from the event page.",
                )}
              </p>
            </div>
          )}
          ...
          <div className="sm:col-span-2">
            <Button type="submit" disabled={busy} className="rounded-full h-11 px-6">
              {busy ? t("יוצר…", "Creating…") : t("יצירת אירוע →", "Create event →")}
            </Button>
          </div>
        </form>
      </Card>
