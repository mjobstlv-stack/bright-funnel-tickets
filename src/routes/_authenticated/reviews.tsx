...
import { listReviewQueue, decideTicketRequest, rescreenTicketRequest } from "@/lib/requests.functions";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          <p className="text-sm text-muted-foreground inline-flex items-center gap-1.5"><ShieldCheck className="h-4 w-4" /> {t("תור אישורים", "Review queue")}</p>
          <h1 className="text-3xl font-display tracking-tight mt-1">{t("אישור מבלים", "Approve attendees")}</h1>
          <p className="text-sm text-muted-foreground mt-1">
            {t("כל הבקשות שה-AI העביר לבדיקה ידנית — במקום אחד, מכל האירועים.", "Every request the AI flagged for manual review — one queue, across all your events.")}
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <Card className="p-10 text-center border-dashed border-black/15 bg-white">
          ...
          <p className="mt-3 font-medium">{t("אין בקשות בקטגוריה זו", "No requests in this category")}</p>
          <p className="text-sm text-muted-foreground mt-1">{t("כשה-AI יסמן מישהו כ'לא בטוח' הוא יופיע כאן.", "When the AI marks someone as ‘Maybe’, they appear here.")}</p>
        </Card>
      ) : (
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {ev && (
                        <Link to="/events/$id" params={{ id: ev.id }} className="text-[11px] px-2 py-0.5 rounded-full bg-black/[0.04] text-muted-foreground hover:bg-black/10">
                          {ev.name}
                        </Link>
                      )}
                    </div>
                    <div className="text-xs text-muted-foreground mt-0.5">
                      {r.buyer_email}{r.buyer_phone ? ` · ${r.buyer_phone}` : ""}
                    </div>
                    <div className="mt-2 flex flex-wrap gap-2 text-xs">
                      <span className="text-muted-foreground self-center">{new Date(r.created_at).toLocaleString()}</span>
                    </div>
                    <div className="mt-2 flex flex-wrap gap-3">
                      {r.instagram_url && <Shot url={r.instagram_url} label="Instagram" icon={<Instagram className="h-3 w-3" />} />}
                      {r.facebook_url && <Shot url={r.facebook_url} label="Facebook" icon={<Facebook className="h-3 w-3" />} />}
                    </div>
                    {r.ai_reasoning && (
                      <p className="mt-2 text-xs text-muted-foreground italic max-w-2xl leading-relaxed">"{r.ai_reasoning}"</p>
                    )}
                  </div>
...
function Shot({ url, label, icon }: { url: string; label: string; icon: React.ReactNode }) {
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="block w-32 rounded-lg overflow-hidden border border-black/10 hover:border-black/30 transition">
      <img src={url} alt={`${label} profile screenshot`} className="w-full h-32 object-cover" />
      <span className="flex items-center gap-1 px-2 py-1 text-[11px] text-muted-foreground">{icon} {label}</span>
    </a>
  );
}
