...
import {
  ...
  type SectionEntry,
  type SectionKey,
} from "@/components/event/EventExtras";
    ...
    return (
      <div className={`${base} bg-white`}>
        <div className="h-full rounded-md grid grid-cols-2 gap-1 overflow-hidden">
          <div className="bg-gradient-to-br from-pink-200 to-sky-200 flex items-center justify-center">
            <div className="h-4 w-4 rounded bg-white/90" />
          </div>
          <div className="bg-neutral-100 p-1.5 flex flex-col gap-1 justify-center">
            <div className="h-1.5 w-10 rounded bg-neutral-400" />
            <div className="h-1 w-8 rounded bg-neutral-300" />
            <div className="h-1 w-6 rounded bg-neutral-300" />
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className={`${base} bg-white`}>
      <div className="h-full rounded-md overflow-hidden border border-neutral-200 flex flex-col">
        <div className="h-1/2 bg-gradient-to-br from-pink-200 to-sky-200 flex items-center justify-center">
          <div className="h-3 w-6 rounded bg-white/90" />
        </div>
...
function CriteriaTester({
  ...
  async function run() {
    if (!criteria.trim()) {
      toast.error(t("כתוב קריטריונים קודם", "Write criteria first"));
      return;
    }
    if (!igShot && !fbShot) {
      toast.error(t("העלה לפחות צילום מסך אחד", "Upload at least one screenshot"));
      return;
    }
    setBusy(true);
    setResult(null);
    ...
    <div className="mt-3 rounded-lg border border-dashed border-black/15 bg-white/60 p-3 space-y-2">
      <div className="flex items-center gap-2">
        ...
        <div className="text-[11px] text-muted-foreground">
          {t("העלה צילום מסך של פרופיל דוגמה", "Upload a screenshot of a sample profile")}
        </div>
      </div>
...
function ShotUpload({
  ...
  function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!f.type.startsWith("image/")) {
      toast.error(t("קובץ לא תקין", "Invalid file"));
      return;
    }
    if (f.size > 6 * 1024 * 1024) {
      toast.error(t("הקובץ גדול מדי (מקס׳ 6MB)", "File too large (max 6MB)"));
      return;
    }
    ...
    <div className="relative">
      ...
      {shot ? (
        <div className="relative rounded-md overflow-hidden border border-black/10 bg-white h-24">
          ...
          <div className="absolute top-1 start-1 text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded bg-black/60 text-white">
            {label}
          </div>
          <button
            type="button"
    ...
    <div className="p-3 space-y-2 text-[11px]">
      <div className="flex items-start gap-1.5 text-amber-800">
        ...
        <div>
          <div className="font-medium leading-tight">
            {t("נדרשת בדיקה ידנית", "Manual review required")}
          </div>
      ...
      {shot ? (
        ...
        >
          ...
          <div className="text-[11px] font-medium text-foreground">
            {t("העלה צילום מסך של הפרופיל", "Upload profile screenshot")}
          </div>
          <div className="text-[10px]">
            {t(
              "פתח/י את הפרופיל, צלמ/י ומחק/י את המדיה הרגישה במידת הצורך",
              "Open the profile, take a screenshot, redact sensitive info if needed",
            )}
          </div>
        </label>
      )}
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        {shot && (
          <label
            htmlFor={inputId}
            className="inline-flex items-center gap-1 h-6 px-2 rounded-full border border-black/15 hover:bg-black/[0.04] cursor-pointer"
          >
      ...
      <ol className="list-decimal ps-4 space-y-0.5 text-muted-foreground leading-snug">
        <li>
          {t(
            "פתח/י את הקישור בכרטיסייה חדשה תוך כניסה לחשבון שלך.",
            "Open the link in a new tab while logged in to your own account.",
          )}
        </li>
        <li>
          {t(
            "בדוק/י שהפרופיל תואם את הקריטריונים שהגדרת.",
            "Check that the profile matches the criteria you set.",
          )}
        </li>
        <li>
          {t(
            "חזור/חזרי לרשימת הבקשות ולחצ/י אשר או דחה בהתאם.",
            "Return to the requests list and press Approve or Reject accordingly.",
          )}
        </li>
      </ol>
      <div className="flex items-center gap-1 text-[10px] text-amber-700">
        ...
        <span>
          {t(
            "הבקשה תסומן ‘לא בטוח’ עד להחלטה ידנית",
            "The request stays marked ‘Maybe’ until you decide",
          )}
        </span>
      </div>
    </div>
...
function EventDetail() {
  ...
  const reload = useCallback(async () => {
    ...
    const { data: ords } = await supabase
      .from("orders")
      .select("id,order_number,buyer_name,buyer_email,total,status,created_at")
      .eq("event_id", id)
    ...
    const { data: rqs } = await supabase
      ...
      .select(
        "id,buyer_name,buyer_email,buyer_phone,instagram_url,facebook_url,status,ai_decision,ai_score,ai_reasoning,created_at",
      )
      .eq("event_id", id)
      .order("created_at", { ascending: false });
  ...
  async function togglePublish() {
    ...
    if (next === "published" && tickets.length === 0) {
      toast.error(
        t(
          "יש להוסיף לפחות סוג כרטיס אחד לפני פרסום",
  ...
  async function deleteEvent() {
    if (
      !confirm(
        t("למחוק את האירוע? לא ניתן לבטל פעולה זו.", "Delete this event? This cannot be undone."),
      )
    )
      return;
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            <span
              className={`px-2 py-0.5 rounded-full text-xs ${ev.status === "published" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}
            >
              {ev.status === "published" ? t("פורסם", "published") : t("טיוטה", "draft")}
            </span>
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" asChild className="rounded-full">
            <a href={`/e/${ev.slug}`} target="_blank" rel="noopener noreferrer">
              <ExternalLink className="h-4 w-4 me-1" /> {t("צפייה בדף", "View page")}
            </a>
          </Button>
      ...
      <Tabs
        value={tab ?? "details"}
        onValueChange={(v) =>
          nav({ to: "/events/$id", params: { id }, search: { tab: v }, replace: true })
        }
      >
        <TabsList>
          <TabsTrigger value="details">{t("פרטים", "Details")}</TabsTrigger>
          <TabsTrigger value="tickets">
            {t("כרטיסים", "Tickets")} ({tickets.length})
          </TabsTrigger>
          {ev.requires_approval && (
            <TabsTrigger value="requests">
              {t("בקשות", "Requests")} ({requests.length})
            </TabsTrigger>
          )}
          <TabsTrigger value="orders">
            {t("הזמנות", "Orders")} ({orders.length})
          </TabsTrigger>
...
function DetailsForm({
  ...
  event: EventT;
  onSave: (p: Partial<EventT>) => Promise<void>;
  onDelete: () => void;
}) {
  ...
  async function runAiGenerate() {
    if (!aiPrompt.trim()) {
      toast.error(t("תאר/י את האירוע קודם", "Describe your event first"));
      return;
    }
    setAiBusy(true);
    try {
      const res = await genPage({
        data: { eventId: event.id, prompt: aiPrompt.trim(), language: aiLang },
      });
      const patch = {
    ...
    try {
      ...
      toast.error(
        err instanceof Error
          ? err.message
          : t("יצירת קישור Meet נכשלה", "Failed to create Meet link"),
      );
    } finally {
      setGenBusy(false);
    }
  ...
  async function handleCoverUpload(file: File) {
    ...
    if (file.size > 8 * 1024 * 1024) {
      toast.error(t("התמונה חייבת להיות עד 8MB", "Image must be under 8MB"));
      return;
    }
    setUploading(true);
    try {
      ...
      if (signed.error || !signed.data)
        throw signed.error ?? new Error(t("שגיאה ביצירת URL", "Could not sign URL"));
  ...
  async function handleCoverVideoUpload(file: File) {
    ...
    if (file.size > 40 * 1024 * 1024) {
      toast.error(t("הסרטון חייב להיות עד 40MB", "Video must be under 40MB"));
      return;
    }
  ...
  async function handleLogoUpload(file: File) {
    ...
    if (file.size > 4 * 1024 * 1024) {
      toast.error(t("הלוגו חייב להיות עד 4MB", "Logo must be under 4MB"));
      return;
    }
  ...
  const urlSchema = z
    .string()
    .trim()
    .url(t("חייב להיות URL תקין", "Must be a valid URL"))
    .max(2048);
  function validate(): Record<string, string> {
    ...
    if (!name) errs.name = t("שם הוא שדה חובה", "Name is required");
      ...
      errs.name = t("שם עד 120 תווים", "Name must be under 120 characters");
    if ((form.tagline ?? "").length > 200)
      errs.tagline = t("סלוגן עד 200 תווים", "Tagline must be under 200 characters");
    if ((form.description ?? "").length > 5000)
      errs.description = t("תיאור עד 5000 תווים", "Description must be under 5000 characters");

    if (form.event_type !== "virtual") {
      if (!(form.venue_name ?? "").trim())
        errs.venue_name = t(
          "שם מיקום נדרש לאירוע פיזי",
    ...
    if (form.event_type !== "physical") {
      ...
      if (!url)
        errs.online_url = t(
          "קישור מפגש נדרש לאירוע אונליין",
          "Meeting link is required for online events",
        );
      else {
        const parsed = urlSchema.safeParse(url);
        if (!parsed.success)
          errs.online_url = parsed.error.issues[0]?.message ?? t("URL לא תקין", "Invalid URL");
      }
    }
    ...
    if (form.video_url && form.video_url.trim()) {
      ...
      if (!parsed.success)
        errs.video_url = parsed.error.issues[0]?.message ?? t("URL לא תקין", "Invalid URL");
    }
  ...
  function handleSave() {
    ...
    onSave({
      ...
      booking_slug:
        form.sale_mode === "booking" ? form.booking_slug?.trim() || venueSlug || null : null,
  ...
  const steps = [
    {
      key: "basics",
      title: t("בסיס", "Basics"),
      hint: t("שם, פורמט, תאריכים", "Name, format, when"),
    },
    {
      key: "where",
      title: form.event_type === "virtual" ? t("אונליין", "Online") : t("מיקום", "Where"),
      hint:
        form.event_type === "virtual"
          ? t("קישור מפגש", "Meeting link")
          : t("מיקום וקישור", "Venue & link"),
    },
      ...
      <ol className="flex items-center gap-3">
        {steps.map((s, i) => (
          <li key={s.key} className="flex items-center gap-3 flex-1">
            ...
            >
              <span
                className={`h-7 w-7 rounded-full grid place-items-center text-xs font-medium transition ${i < step ? "bg-black text-white" : i === step ? "bg-black text-white" : "bg-black/5 text-black/50"}`}
              >
      ...
      <Card className="p-6 border-black/10">
          ...
          <div className="space-y-5 max-w-2xl">
            <Field label={t("שם האירוע", "Event name")} required error={errors.name}>
              <Input
                ...
                maxLength={120}
                placeholder={t("למשל: מסיבת גג קיץ", "e.g. Summer Rooftop Party")}
                value={form.name}
            ...
            <div className="rounded-2xl border border-black/10 p-5 bg-gradient-to-br from-amber-50 via-white to-emerald-50">
              <div className="font-medium text-sm">
                {t("מה מוכרים בדף האירוע?", "What does the event page sell?")}
              </div>
              <div className="mt-3 grid sm:grid-cols-2 gap-2">
                {[
                  {
                    k: "tickets" as SaleMode,
                    title: t("מכירת כרטיסים", "Ticket sales"),
                    hint: t(
                      "סוגי כרטיסים, כמויות ותשלום מלא",
                      "Ticket tiers, quantities and full payment",
                    ),
                  },
                  {
                    k: "booking" as SaleMode,
                    title: t("שמירת מקום (שולחנות)", "Table booking"),
                    hint: t(
                      "בחירת סועדים, תאריך ושעה — כמו במסעדה",
                      "Party size, date and time — like the restaurant module",
                    ),
                  },
                ].map((o) => (
                  <button
              ...
              {(form.sale_mode ?? "tickets") === "booking" && (
                <div className="mt-4 space-y-3">
                  <Field label={t("קישור ישיר להזמנות", "Direct booking link")}>
                    <Input
                      ...
                      onChange={(e) =>
                        setForm({
                          ...form,
                          booking_slug: e.currentTarget.value
                            .toLowerCase()
                            .replace(/[\s/\\]+/g, "-")
                            .replace(/[^a-z0-9._-]/g, ""),
                        })
                  ...
                  <p className="text-xs text-muted-foreground" dir="ltr">
                    /r/{form.booking_slug ?? venueSlug ?? "my-venue"}
                  </p>
                  {!venueSlug && (
                    <p className="text-xs text-amber-700">
                      {t(
                        "לא נמצאו הגדרות שמירת מקום לעסק — הגדירו אותן במודול המסעדות.",
                        "No reservation settings found for this business — set them up in the Reservations module.",
                      )}
                    </p>
                  )}
                  <div className="flex items-start justify-between gap-3 rounded-xl border border-black/10 bg-white p-3">
                    <div>
                      <div className="text-sm font-medium">
                        {t("גביית פיקדון", "Take a deposit")}
                      </div>
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {t(
                          "פיקדון בכרטיס אשראי לכל סועד. כבוי = שמירת מקום בלי תשלום.",
                          "Credit-card deposit per guest. Off = booking with no payment.",
                        )}
                      </p>
                    </div>
                    <Switch
                  ...
                  {form.booking_deposit_enabled && (
                    <Field label={`${t("פיקדון לסועד", "Deposit per guest")} (${event.currency})`}>
                      <Input
                        ...
                        onChange={(e) =>
                          setForm({ ...form, booking_deposit_per_guest: Number(e.target.value) })
                        }
                      />
                    </Field>
                  )}
            ...
            <div className="rounded-2xl border border-black/10 p-5 bg-gradient-to-br from-fuchsia-50 via-white to-sky-50">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="font-medium text-sm">
                    {t("אישור משתתפים לפי פרופיל חברתי", "Attendee approval by social profile")}
                  </div>
                  <p className="text-xs text-muted-foreground mt-1 max-w-md">
                    {t(
                      "אורחים יגישו בקשה עם פרופיל אינסטגרם/פייסבוק. AI יסנן ראשונית לפי הקריטריונים שלך, ותוכל/י לאשר או לדחות ידנית. תשלום רק אחרי אישור.",
                ...
                <div className="mt-4 space-y-3">
                  <div className="flex flex-wrap gap-4">
                    <label className="inline-flex items-center gap-2 text-sm">
                      <input
                        type="checkbox"
                        checked={!!form.require_instagram}
                        onChange={(e) => setForm({ ...form, require_instagram: e.target.checked })}
                      />
                    ...
                    <label className="inline-flex items-center gap-2 text-sm">
                      <input
                        type="checkbox"
                        checked={!!form.require_facebook}
                        onChange={(e) => setForm({ ...form, require_facebook: e.target.checked })}
                      />
                  ...
                  <div>
                    <Label className="mb-1.5 block text-xs">
                      {t(
                        "קריטריונים לאישור (מה AI ישקול)",
                        "Approval criteria (what AI should consider)",
                      )}
                    </Label>
                    <textarea
                      rows={4}
                      ...
                      placeholder={t(
                        "למשל: גילאים 21+, קהל ת\"א/נייטלייף, לא לאשר פרופילים ריקים או ספאמיים, להעדיף אמנים/יצרני תוכן. פרופילים דו-משמעיים לסמן 'לא בטוח'.",
                    ...
                    <p className="text-[11px] text-muted-foreground mt-1">
                      {t(
                        "שים לב: ה-AI לא סורק את הפרופיל בפועל — הוא מחליט לפי ה-URL/שם משתמש והקריטריונים. פרופילים לא ברורים מסומנים 'לא בטוח' לאישור ידני.",
                  ...
                  <div>
                    <Label className="mb-1.5 block text-xs">
                      {t(
                        "קריטריונים ויזואליים (מה AI יחפש/יפסול בתמונות)",
                        "Visual criteria (what AI should look for / reject in photos)",
                      )}
                    </Label>
                    <textarea
                      rows={3}
                      ...
                      placeholder={t(
                        "למשל: לבוש אלגנטי/סטייליסטי, אווירת נייטלייף, גילאי 25–40, לפסול תמונות אלימות/גזעניות, לא לאשר פרופילים ריקים או ילדותיים.",
                    ...
                    <p className="text-[11px] text-muted-foreground mt-1">
                      {t(
                        "AI יצלם את דף האינסטגרם/פייסבוק (רק פרופילים ציבוריים) וישווה לתיאור. פרופיל פרטי / חסום ייבדק לפי טקסט בלבד.",
                  ...
                  <CriteriaTester
                    criteria={form.approval_criteria ?? ""}
                    visualCriteria={form.visual_criteria ?? ""}
                    eventName={form.name ?? ""}
                  />
                </div>
              )}
        ...
        {step === 1 && (
          <div className="space-y-5 max-w-2xl">
            {form.event_type !== "virtual" && (
              <>
                <Field label={t("שם המיקום", "Venue name")} required error={errors.venue_name}>
                  <Input
                    aria-invalid={!!errors.venue_name}
                    placeholder={t("למשל: The Block", "e.g. The Block")}
                  ...
                  <AddressAutocomplete
                    ...
                    onSelect={(v: string) =>
                      setForm({
                        ...form,
                        venue_address: v,
                        location_info: { ...(form.location_info ?? {}), address: v },
                      })
                    }
                  />
                </Field>
            ...
            {form.event_type !== "physical" && (
              <div className="rounded-xl border border-black/10 p-4 bg-gradient-to-br from-pink-50 to-sky-50">
                <div className="flex items-center gap-2 text-sm font-medium">
                  <Video className="h-4 w-4" /> {t("מפגש אונליין", "Online meeting")}
                </div>
                <div className="mt-3 grid sm:grid-cols-[1fr_auto] gap-2 items-start">
                  ...
                  >
                    ...
                    {genBusy ? t("יוצר…", "Creating…") : t("צור Meet אוטומטית", "Auto-create Meet")}
                  </Button>
                </div>
                {errors.online_url && (
                  <p className="text-xs text-red-600 mt-2">{errors.online_url}</p>
                )}
              </div>
            )}
          </div>
          ...
          <div className="space-y-5 max-w-2xl">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="font-medium">
                      {t("בנה דף נחיתה עם AI", "Build landing page with AI")}
                    </div>
                    <div className="text-xs text-muted-foreground max-w-md">
                      {t(
                        "תאר/י את האירוע בכמה מילים ו-AI יבחר טמפלייט ויכתוב כותרת, סלוגן ותיאור.",
                        "Describe your event in a few words and AI will pick a template and write the headline, tagline and description for you.",
                      )}
                    </div>
                  </div>
                </div>
              ...
              {aiOpen && (
                <div className="mt-4 space-y-3">
                  <div className="flex gap-2">
                    {(["en", "he"] as const).map((l) => (
                      <button
                        ...
                        type="button"
                        onClick={() => setAiLang(l)}
                        className={`h-8 px-3 rounded-full text-xs border transition ${aiLang === l ? "bg-black text-white border-black" : "border-black/15 hover:border-black/40"}`}
                      >
                        {l === "en" ? "English" : "עברית"}
                      </button>
                    ))}
                  ...
                  <textarea
                    ...
                    placeholder={
                      aiLang === "he"
                        ? "תאר/י את האירוע: סוג, קהל, מיקום, אווירה, ואומנים..."
                        : "Describe your event: type, audience, location, vibe, lineup..."
                    }
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    >
                      <Sparkles className="h-4 w-4 me-1" />
                      {aiBusy ? t("יוצר…", "Generating…") : t("צור דף", "Generate page")}
                    </Button>
                  </div>
                  <p className="text-[11px] text-muted-foreground">
                    {t(
                      "טיפ: סלוגן, תיאור וטמפלייט קיימים יוחלפו.",
                      "Tip: existing tagline, description and template will be replaced.",
                    )}
                  </p>
                </div>
              )}
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mb-3">
                {t(
                  "בחר/י איך ייראה דף האירוע הציבורי.",
                  "Choose how the public event page is laid out.",
                )}
              </p>
                ...
                {TEMPLATES.map((tpl) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        setForm({ ...form, template: tpl.key });
                        onSave({ template: tpl.key });
                      }}
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mb-3">
                {t(
                  "החלף את הרקע הדיפולטי של הטמפלייט. צבע הטקסט מתאים אוטומטית או בחר/י שלך.",
                  "Override the template's default background. Text color adjusts automatically, or pick your own.",
                )}
              </p>
              <BgColorPicker
                ...
                >
                  ...
                  {form.logo_url && (
                    <img
                      src={form.logo_url}
                      alt="Logo preview"
                      className="absolute inset-0 m-auto h-3/5 max-h-[80%] w-auto max-w-[70%] object-contain drop-shadow-lg"
                    />
                  )}
                  {form.cover_url && (
                    <button
                      ...
                      onClick={() => {
                        ...
                        onSave({ cover_url: null });
                      }}
                      className="absolute top-1.5 end-1.5 h-6 w-6 rounded-full bg-white/90 hover:bg-white grid place-items-center shadow"
                    >
            ...
            <div>
              <Label className="mb-1.5 block">
                ...
                <span className="text-xs text-muted-foreground font-normal">
                  {t(
                    "(אופציונלי, מתנגן אוטומטית ללא סאונד מעל הקאבר)",
                    "(optional, autoplays muted over the cover)",
                  )}
                </span>
              </Label>
              <div className="flex items-start gap-4">
                  ...
                  {form.cover_video_url ? (
                    <>
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          setForm({ ...form, cover_video_url: null });
                          onSave({ cover_video_url: null });
                        }}
                        className="absolute top-1.5 end-1.5 h-6 w-6 rounded-full bg-white/90 hover:bg-white grid place-items-center shadow"
                      >
                        <X className="h-3.5 w-3.5" />
            ...
            <div>
              <Label className="mb-1.5 block">
                {t("לוגו על גבי הבאנר", "Logo over banner")}{" "}
                <span className="text-xs text-muted-foreground font-normal">
                  {t(
                    "(אופציונלי, מוצג מעל תמונת הבאנר)",
                    "(optional, shown over the banner image)",
                  )}
                </span>
              </Label>
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground max-w-[220px]">
                    {t(
                      "מומלץ PNG שקוף או SVG. מופיע מעל תמונת הבאנר.",
                      "Transparent PNG or SVG recommended. Appears over the banner image.",
                    )}
                  </p>
                </div>
              </div>
            ...
            <div>
              <Label className="mb-1.5 block">
                {t("לוגו בעיגול", "Circle logo")}{" "}
                <span className="text-xs text-muted-foreground font-normal">
                  {t(
                    "(אופציונלי, עיגול לבן ממורכז בין הבאנר לכותרת)",
                    "(optional, white circle between banner and title)",
                  )}
                </span>
              </Label>
              <div className="flex items-start gap-4">
                  ...
                  {form.circle_logo_url ? (
                    <>
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          setForm({ ...form, circle_logo_url: null });
                          onSave({ circle_logo_url: null });
                        }}
                        className="absolute top-1.5 end-1.5 h-6 w-6 rounded-full bg-white/90 hover:bg-white grid place-items-center shadow"
                      >
                        <X className="h-3.5 w-3.5" />
                ...
                <div className="space-y-2">
                  <label className="inline-flex items-center gap-2 h-9 px-4 rounded-full border border-black/15 hover:border-black/40 text-sm cursor-pointer">
                    ...
                    <input
                      ...
                      accept="image/png,image/svg+xml,image/webp,image/jpeg"
                      className="hidden"
                      disabled={uploadingCircleLogo}
                      onChange={(e) => {
                        const f = e.target.files?.[0];
                        if (f) handleCircleLogoUpload(f);
                        e.target.value = "";
                      }}
                    />
                  </label>
                  <p className="text-xs text-muted-foreground max-w-[220px]">
                    {t(
                      "מומלץ קובץ ריבועי. מוצג בעיגול לבן על גבול הבאנר.",
                      "Square file recommended. Shown in a white circle on the banner edge.",
                    )}
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-black/10 p-4">
                ...
                <div className="mt-3 flex items-center gap-2">
                  <Button
                    type="button"
                    size="sm"
                    onClick={() => onSave({ logo_layout: form.logo_layout })}
                  >
                    {t("שמור מיקום", "Save placement")}
                  </Button>
                  <span className="text-xs text-muted-foreground">
                    {t("שמור/י כדי להחיל על הדף הציבורי", "Save to apply on the public page")}
                  </span>
                </div>
              </div>
            )}
            <Field label={t("סלוגן", "Tagline")} error={errors.tagline}>
              <Input
                aria-invalid={!!errors.tagline}
                maxLength={200}
                placeholder={t("שורה אחת שמופיעה מתחת לכותרת", "One-liner shown under the title")}
                value={form.tagline ?? ""}
            ...
            <Field label={t("תיאור", "Description")} error={errors.description}>
              <textarea
                ...
                placeholder={t("ספר/י על מה האירוע", "Tell people what this event is about")}
        ...
        <div className="flex items-center justify-between pt-6 mt-6 border-t border-black/5">
          {step === 0 ? (
            <Button variant="ghost" className="text-red-600 hover:text-red-700" onClick={onDelete}>
              <Trash2 className="h-4 w-4 me-1" /> {t("מחיקה", "Delete")}
            </Button>
          ) : (
            <Button variant="outline" className="rounded-full" onClick={() => setStep(step - 1)}>
              <ChevronLeft className="h-4 w-4 me-1 rtl:rotate-180" /> {t("חזור", "Back")}
            </Button>
          )}
          <Button className="rounded-full" onClick={next}>
            {step < steps.length - 1 ? (
              <>
                {t("הבא", "Next")} <ChevronRight className="h-4 w-4 ms-1 rtl:rotate-180" />
              </>
            ) : (
...
function ExtrasEditor({
  ...
  async function runParse() {
    if (!pasteText.trim()) {
      toast.error(t("הדבק/י טקסט קודם", "Paste some text first"));
      return;
    }
    setPasteBusy(true);
    try {
  ...
  function applyPastePreview() {
    ...
    onChange({
      includes: merge
        ? [...includes, ...pastePreview.includes].slice(0, 12)
      ...
      location_info: merge
        ? { ...location, ...stripEmpty(pastePreview.location) }
  ...
  async function importScheduleFromExcel(file: File) {
    try {
      ...
      if (items.length === 0) {
        toast.error(t("לא נמצאו שורות בקובץ", "No rows found in file"));
        return;
      }
    ...
    <div className="space-y-5 pt-4 mt-4 border-t border-black/5">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <Label className="text-sm">
            {t("סקשנים נוספים", "Extra sections")}{" "}
            <span className="text-xs text-muted-foreground font-normal">
              {t("(הכל אופציונלי)", "(all optional)")}
            </span>
          </Label>
          <p className="text-xs text-muted-foreground mt-0.5">
            {t(
              "הדגשים, מה כלול, מיקום, תוכנייה, שאלות נפוצות, נהלים וסרטון — הכל אופציונלי.",
              "Highlights, what's included, location, schedule, FAQ, rules and a promo video — all optional.",
            )}
          </p>
        </div>
        <Button
      ...
      <div className="rounded-2xl border border-black/10 p-4 bg-white">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {t(
                "סדר את הסקשנים בדף האירוע. כבה/הפעל כל סקשן.",
                "Reorder sections on the event page. Toggle any section on/off.",
              )}
            </p>
          </div>
          <Button
            ...
            variant="ghost"
            size="sm"
            onClick={() => updateSections(DEFAULT_SECTIONS.map((s) => ({ ...s })))}
          >
            {t("איפוס", "Reset")}
          </Button>
        </div>
        <ul className="mt-3 divide-y divide-black/5 border border-black/10 rounded-xl overflow-hidden">
          {orderedSections.map((s, i) => (
            <li
              key={s.key}
              className={`flex items-center gap-2 px-3 py-2 ${s.enabled ? "bg-white" : "bg-black/[0.02]"}`}
            >
              <span
                className={`text-xs w-5 text-center tabular-nums ${s.enabled ? "text-muted-foreground" : "text-muted-foreground/60"}`}
              >
                {i + 1}
              </span>
              <span
                className={`flex-1 text-sm ${s.enabled ? "" : "text-muted-foreground line-through"}`}
              >
                {SECTION_LABELS[s.key][lang === "he" ? "he" : "en"]}
              </span>
              <Button
                type="button"
                variant="ghost"
              ...
              <Button
                ...
                aria-label={s.enabled ? t("כבה", "Hide") : t("הפעל", "Show")}
              >
                {s.enabled ? (
        ...
        <p className="text-[11px] text-muted-foreground mt-2">
          {t(
            "סקשן ריק (בלי תוכן) לא יופיע בדף — גם אם הוא מופעל כאן.",
            "Empty sections (with no content) don't appear on the page — even if enabled here.",
          )}
        </p>
      </div>
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground max-w-md">
                {t(
                  "הדבק/י תיאור ארוך (בכל שפה) ו-AI ימיין ל-מה כלול, מיקום, שאלות נפוצות ונהלים.",
                  "Paste a long description (any language) and AI will sort it into Includes, Location, FAQ and Rules.",
                )}
              </div>
            </div>
          </div>
        ...
        {pasteOpen && (
          <div className="mt-4 space-y-3">
            <textarea
              ...
              placeholder={
                "הדבק/י כאן את כל הטקסט שכתבת בעבר על האירוע — מה כלול, איך מגיעים, שאלות נפוצות, נהלים…"
              }
              ...
              <div className="flex gap-2">
                {(["append", "replace"] as const).map((m) => (
                  <button
                    ...
                    className={`h-8 px-3 rounded-full text-xs border transition ${mergeMode === m ? "bg-black text-white border-black" : "border-black/15 hover:border-black/40"}`}
                  >
                    {m === "append"
                      ? t("הוסף לקיים", "Add to existing")
                      : t("החלף את הקיים", "Replace existing")}
                  </button>
                ))}
              </div>
              <div className="flex gap-2">
                ...
                >
                  ...
                  {pasteBusy
                    ? t("מארגן…", "Organizing…")
                    : pastePreview
                      ? t("הרץ AI שוב", "Re-run AI")
                      : t("ארגן עם AI", "Organize with AI")}
                </Button>
              </div>
            </div>
            {pastePreview && (
                ...
                <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
                  <div>
                    <div className="text-[11px] uppercase tracking-widest text-muted-foreground">
                      {t(
                        "תצוגה מקדימה — איך זה יופיע בדף הציבורי",
                        "Preview — how it will appear on the public page",
                      )}
                    </div>
                    <div className="text-xs text-muted-foreground mt-0.5">
                      {mergeMode === "append"
                        ? t(
                            "יתווסף לסקשנים הקיימים שלך.",
                            "Will be added to your existing sections.",
                          )
                        : t(
                            "יחליף את הסקשנים הקיימים שלך.",
                            "Will replace your existing sections.",
                          )}
                    </div>
                  </div>
                  <div className="flex gap-2">
                    ...
                    >
                      {mergeMode === "append"
                        ? t("החל (הוסף)", "Apply (add)")
                        : t("החל (החלף)", "Apply (replace)")}
                    </Button>
                  </div>
                </div>
                <EventExtras
      ...
      <div className="rounded-2xl border border-black/10 p-4 bg-white">
        <div className="flex items-center justify-between">
          <div className="text-sm font-medium">
            {t("הדגשים", "Highlights")}{" "}
            <span className="text-xs text-muted-foreground font-normal">
              ({highlights.length}/6 {t("אייקונים", "icons")})
            </span>
          </div>
          <Button
            type="button"
        ...
        {highlights.length === 0 ? (
          <p className="text-xs text-muted-foreground mt-2">
            {t(
              "אין הדגשים עדיין. הוסף/י 4–6 אייקונים שמתארים מה מחכה למוזמנים.",
              "No highlights yet. Add 4–6 icons that describe what guests can expect.",
            )}
          </p>
        ) : (
            ...
            {highlights.map((h, i) => (
              <div key={i} className="grid grid-cols-[140px_1fr_auto] gap-2 items-start">
                ...
                <div className="space-y-1.5">
                  <IconTextField
                    placeholder={t("כותרת", "Title")}
                    maxLength={60}
                    value={h.title}
                    onChange={(v) => patchHighlight(i, { title: v })}
                    userId={userId}
                    scopeId={eventId}
                  />
                  <IconTextField
                    placeholder={t("תיאור קצר (אופציונלי)", "Short description (optional)")}
                    maxLength={140}
                    value={h.description ?? ""}
                    onChange={(v) => patchHighlight(i, { description: v })}
                    userId={userId}
                    scopeId={eventId}
                  />
                </div>
                <Button
                  ...
                  aria-label={t("הסר הדגש", "Remove highlight")}
                >
      ...
      <div className="rounded-2xl border border-black/10 p-4 bg-white">
        ...
        {includes.length === 0 ? (
          <p className="text-xs text-muted-foreground mt-2">
            {t(
              "מה כרטיס מקנה — שתייה, אוכל, מוזיקה, חניה…",
              "List what the ticket gets you — drinks, food, music, parking…",
            )}
          </p>
        ) : (
          <div className="mt-3 space-y-2">
            {includes.map((it, i) => (
              <div key={i} className="grid grid-cols-[140px_1fr_auto] gap-2 items-center">
                ...
                />
                <IconTextField
                  placeholder={t("למשל: כוס שתייה בכניסה", "e.g. Welcome drink on arrival")}
                  maxLength={200}
                  value={it.text}
                  onChange={(v) => patchInclude(i, { text: v })}
                  userId={userId}
                  scopeId={eventId}
                />
                <Button
      ...
      <div className="rounded-2xl border border-black/10 p-4 bg-white">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          {t("איפה זה קורה, איך מגיעים, חניה.", "Where it happens, how to get there, parking.")}
        </p>
        <div className="mt-3 grid sm:grid-cols-2 gap-3">
          ...
          <Field label={t("קישור למפה", "Map URL")}>
            <Input
              type="url"
              placeholder="https://maps.google.com/…"
              value={location.map_url ?? ""}
              onChange={(e) => patchLocation({ map_url: e.target.value })}
            />
          </Field>
          <Field label={t("חניה", "Parking")}>
            <textarea
              ...
              onChange={(e) => patchLocation({ parking: e.target.value })}
              className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
            />
          </Field>
          <Field label={t("תחבורה ציבורית", "Public transit")}>
            <textarea
              rows={2}
              maxLength={500}
              value={location.transit ?? ""}
              onChange={(e) => patchLocation({ transit: e.target.value })}
              className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
            />
          </Field>
          <div className="sm:col-span-2">
            <Field label={t("הערות", "Notes")}>
              <textarea
                ...
                onChange={(e) => patchLocation({ notes: e.target.value })}
                className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
              />
            </Field>
          </div>
      ...
      <div className="rounded-2xl border border-black/10 p-4 bg-white">
        ...
        {schedule.length === 0 ? (
          <p className="text-xs text-muted-foreground mt-2">
            {t(
              "אין תוכנייה עדיין. הוסף/י שורה או ייבא/י מאקסל (עמודות: שעה, כותרת, הערות).",
              "No schedule yet. Add a row or import an Excel file (columns: time, title, notes).",
            )}
          </p>
        ) : (
          <div className="mt-3 space-y-2">
            {schedule.map((s, i) => (
              <div key={i} className="grid grid-cols-[110px_1fr_auto] gap-2 items-start">
                <Input
                  placeholder="20:30"
                  value={s.time ?? ""}
                  onChange={(e) => patchSchedule(i, { time: e.target.value })}
                />
                <div className="space-y-1.5">
                  <Input
                    placeholder={t("כותרת (למשל: פתיחת דלתות)", "Title (e.g. Doors open)")}
                    maxLength={120}
                    value={s.title}
                    onChange={(e) => patchSchedule(i, { title: e.target.value })}
                  />
                  <Input
                    placeholder={t("הערות (אופציונלי)", "Notes (optional)")}
                    maxLength={200}
                    value={s.description ?? ""}
                    onChange={(e) => patchSchedule(i, { description: e.target.value })}
                  />
                </div>
                <Button
                  type="button"
                  ...
                  aria-label={t("הסר שורה", "Remove row")}
                >
                  <Trash2 className="h-4 w-4 text-red-600" />
                </Button>
              </div>
      ...
      <div className="rounded-2xl border border-black/10 p-4 bg-white">
        <div className="flex items-center justify-between">
          <div className="text-sm font-medium">
            {t("שאלות נפוצות", "FAQ")}{" "}
            <span className="text-xs text-muted-foreground font-normal">({faq.length}/12)</span>
          </div>
          <Button
            type="button"
            variant="ghost"
        ...
        {faq.length === 0 ? (
          <p className="text-xs text-muted-foreground mt-2">
            {t(
              "שאלות נפוצות — החזרים, קוד לבוש, כשרות, גיל, וכו׳.",
              "Frequently asked questions — refunds, dress code, kosher, age limit, etc.",
            )}
          </p>
        ) : (
            ...
            {faq.map((f, i) => (
              <div key={i} className="grid grid-cols-[1fr_auto] gap-2 items-start">
                ...
                <Button
                  ...
                  size="icon"
                  onClick={() => removeFaq(i)}
                  aria-label={t("הסר שאלה", "Remove FAQ")}
                >
                  <Trash2 className="h-4 w-4 text-red-600" />
                </Button>
              </div>
      ...
      <div className="rounded-2xl border border-black/10 p-4 bg-white">
        <div className="flex items-center justify-between">
          <div className="text-sm font-medium">
            {t("נהלי האירוע", "Event rules")}{" "}
            <span className="text-xs text-muted-foreground font-normal">({rules.length}/12)</span>
          </div>
          <Button
            type="button"
            variant="ghost"
        ...
        {rules.length === 0 ? (
          <p className="text-xs text-muted-foreground mt-2">
            {t(
              "נקודות קצרות — מדיניות ביטול, ת.ז. בכניסה, כניסה מאוחרת, עישון, וכו׳.",
              "Short bullets — cancellation policy, ID at entry, late entry, smoking, etc.",
            )}
          </p>
        ) : (
          <div className="mt-3 space-y-2">
            {rules.map((r, i) => (
              <div key={i} className="grid grid-cols-[1fr_auto] gap-2 items-start">
                <IconTextField
                  placeholder={t("נהל קצר", "Short rule")}
                  maxLength={280}
                  value={r}
                  onChange={(v) => patchRule(i, v)}
                  userId={userId}
                  scopeId={eventId}
                />
                <Button
                  ...
                  aria-label={t("הסר נהל", "Remove rule")}
                >
                  <Trash2 className="h-4 w-4 text-red-600" />
                </Button>
              </div>
      ...
      <div className="rounded-2xl border border-black/10 p-4 bg-white">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          {t("הדבק/י קישור YouTube, Vimeo או MP4.", "Paste a YouTube, Vimeo, or direct MP4 URL.")}
        </p>
        <div className="mt-2">
          <Input
            type="url"
      ...
      <div className="rounded-2xl border border-black/10 p-4 bg-white">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {t(
                "העלה תמונות שמופיעות בגלריה בדף האירוע.",
                "Upload images to show in a gallery on the event page.",
              )}
            </p>
          </div>
          <label
            className={`inline-flex items-center gap-2 h-9 px-4 rounded-full border text-sm cursor-pointer ${uploadingGallery ? "opacity-60" : "border-black/15 hover:border-black/40"}`}
          >
            ...
            <input
              ...
              accept="image/*"
              className="hidden"
              disabled={uploadingGallery || gallery.length >= 24}
              onChange={(e) => {
        ...
        {gallery.length > 0 && (
          <div className="mt-3 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
            {gallery.map((src, i) => (
              <div
                key={i}
                className="relative aspect-square rounded-lg overflow-hidden border border-black/10 bg-black/5 group"
              >
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-end justify-between p-1 opacity-0 group-hover:opacity-100">
                  <div className="flex gap-1">
                    <button
                      ...
                      disabled={i === 0}
                      className="h-6 w-6 rounded bg-white/90 text-black grid place-items-center text-xs disabled:opacity-40"
                      aria-label={t("הזז שמאלה", "Move left")}
                    >
                      ‹
                    </button>
                    <button
                      ...
                      className="h-6 w-6 rounded bg-white/90 text-black grid place-items-center text-xs disabled:opacity-40"
                      aria-label={t("הזז ימינה", "Move right")}
                    >
                      ›
                    </button>
                  </div>
                  <button
                    ...
                    className="h-6 w-6 rounded bg-red-600 text-white grid place-items-center"
                    aria-label={t("מחק", "Delete")}
                  >
                    <X className="h-3.5 w-3.5" />
                  </button>
                </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button onClick={() => setCreating((v) => !v)} className="rounded-full">
          <Plus className="h-4 w-4 me-1" /> {t("הוסף סוג כרטיס", "Add ticket type")}
        </Button>
      </div>
      {creating && (
        <Card className="p-5 border-black/10 grid sm:grid-cols-4 gap-3 items-end">
          <div className="sm:col-span-4">
            ...
            <div className="mt-1.5 grid grid-cols-2 gap-2 max-w-md">
              {[
                ...
                {
                  k: "reservation" as const,
                  he: "שמירת מקום + פיקדון",
            ...
            {form.kind === "reservation" && (
              <p className="text-xs text-muted-foreground mt-2">
                {t(
                  "האורח משלם פיקדון בכרטיס אשראי כדי לשמור מקום, והיתרה נגבית במקום.",
                  "The guest pays a card deposit to hold the spot; the balance is charged on site.",
                )}
              </p>
            )}
          </div>
          <Field label={t("שם", "Name")}>
            <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
          </Field>
          <Field
            label={`${form.kind === "reservation" ? t("מחיר מלא", "Full price") : t("מחיר", "Price")} (${currency})`}
          >
            <Input
              type="number"
              min={0}
          ...
          <div className="sm:col-span-4">
            <Field label={t("תיאור", "Description")}>
              <Input
                value={form.description}
                onChange={(e) => setForm({ ...form, description: e.target.value })}
              />
          ...
          {form.kind === "reservation" && (
            <div className="sm:col-span-4 flex items-center gap-2">
              <Switch
                checked={form.balance_on_site}
                onCheckedChange={(v) => setForm({ ...form, balance_on_site: v })}
              />
              <span className="text-xs text-muted-foreground">
                {t("היתרה נגבית במקום בכרטיס אשראי", "Remaining balance charged on site by card")}
              </span>
            </div>
          )}
        </Card>
      ...
      {tickets.length === 0 ? (
        <Card className="p-8 text-center border-dashed border-black/15 text-sm text-muted-foreground">
          {t(
            "אין עדיין סוגי כרטיסים. הוסף/י אחד כדי להתחיל למכור.",
          ...
          {tickets.map((tp) => (
            <div key={tp.id} className="p-4 flex flex-wrap items-center gap-4">
              <div className="flex-1 min-w-[180px]">
                <Input
                  value={tp.name}
                  onChange={(e) => patch(tp.id, { name: e.target.value })}
                  className="font-medium border-transparent hover:border-input"
                />
                {tp.description && (
                  <p className="text-xs text-muted-foreground mt-0.5 px-3">{tp.description}</p>
                )}
                      ...
                      onClick={() =>
                        patch(
                          ...
                          o.k === "reservation"
                            ? {
                                kind: o.k,
                                deposit_amount: Number(tp.deposit_amount) || 0,
                                balance_on_site: true,
                              }
              ...
              <div className="w-28">
                ...
                <Input
                  ...
                  step="0.01"
                  value={tp.price}
                  onChange={(e) => patch(tp.id, { price: Number(e.target.value) })}
                />
              </div>
              {(tp.kind ?? "ticket") === "reservation" && (
                <div className="w-28">
                  <Label className="text-xs">
                    {t("פיקדון", "Deposit")} ({currency})
                  </Label>
                  <Input
                    type="number"
                    min={0}
                    step="0.01"
                    value={Number(tp.deposit_amount) || 0}
                    onChange={(e) => patch(tp.id, { deposit_amount: Number(e.target.value) })}
                  />
                </div>
              )}
              <div className="w-36">
                ...
                <div className="text-sm h-9 flex items-center gap-1">
                  ...
                  <Input
                    type="number"
                    min={0}
                    value={tp.quantity_total}
                    onChange={(e) => patch(tp.id, { quantity_total: Number(e.target.value) })}
                    className="w-20 h-8"
                  />
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Switch
                  ...
                  onCheckedChange={(v) => patch(tp.id, { is_active: v })}
                />
                <span className="text-xs text-muted-foreground">
                  {tp.is_active ? t("פעיל", "Active") : t("כבוי", "Off")}
                </span>
              </div>
              <Button variant="ghost" size="icon" onClick={() => remove(tp.id)}>
                <Trash2 className="h-4 w-4 text-red-600" />
              </Button>
            </div>
          ))}
        ...
        <tbody className="divide-y divide-black/5">
          {orders.map((o) => (
            <tr key={o.id}>
              ...
              <td className="px-4 py-2">
                {currency} {Number(o.total).toLocaleString()}
              </td>
              <td className="px-4 py-2">
                <span
                  className={`px-2 py-0.5 rounded-full text-xs ${o.status === "paid" ? "bg-emerald-100 text-emerald-700" : o.status === "pending" ? "bg-amber-100 text-amber-700" : "bg-gray-100 text-gray-700"}`}
                >
                  {o.status === "paid"
                    ? t("שולם", "paid")
                    : o.status === "pending"
          ...
          <input
            ...
            onChange={(e) =>
              onChange({
                ...
                text_color: text ?? readableTextOn(e.target.value),
              })
            }
...
function StatusPill({ status }: { status: RequestRow["status"] }) {
  ...
  const m = map[status];
  return (
    <span className={`px-2 py-0.5 rounded-full text-xs ${m.cls}`}>{t(m.label.he, m.label.en)}</span>
  );
}
...
function RequestsTab({
  ...
  const { t } = useLang();
  const [filter, setFilter] = useState<"maybe" | "approved" | "rejected" | "pending" | "all">(
    "maybe",
  );
  ...
  if (requests.length === 0) {
    return (
      <Card className="p-8 text-center border-dashed border-black/15 text-sm text-muted-foreground">
        {t(
          "עדיין אין בקשות. שתפ/י את קישור האירוע כדי לקבל בקשות לאישור.",
          "No requests yet. Share your event link to start receiving approval requests.",
        )}
      </Card>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        {tabs.map((tb) => (
          ...
          >
            {tb.label}{" "}
            <span className={filter === tb.key ? "opacity-70" : "text-muted-foreground"}>
              ({tb.count})
            </span>
          </button>
        ))}
            ...
            <Card key={r.id} className="p-4 border-black/10">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div className="min-w-0">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 text-xs">
                    {r.instagram_url && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1 px-2 py-1 rounded-full border border-black/10 hover:bg-black/[0.03]"
                      >
                        Instagram ↗
                      </a>
                    )}
                  ...
                  {r.ai_reasoning && (
                    <p className="mt-2 text-xs text-muted-foreground italic max-w-xl leading-relaxed">
                      "{r.ai_reasoning}"
                    </p>
                  )}
                </div>
