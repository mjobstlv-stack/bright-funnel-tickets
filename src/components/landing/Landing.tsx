...
export function Landing({ lang }: Props) {
  ...
  return (
    <div
      dir={c.dir}
      lang={c.langCode}
      className="min-h-screen bg-cream text-ink overflow-x-hidden font-sans"
      style={{ fontSize: "calc(1rem * var(--a11y-scale, 1))" }}
    >
      <style>{`
...
function Pill({
  ...
  const styles =
    tone === "solid"
      ? "bg-ink text-cream hover:bg-leaf"
      : "border-2 border-ink text-ink hover:bg-ink/5";
  return (
    ...
    <header className="sticky top-0 z-50 bg-cream/90 backdrop-blur-xl border-b border-ink/10">
      <div className="mx-auto max-w-7xl px-5 sm:px-8 h-20 flex items-center justify-between gap-6">
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm font-medium text-ink/70">
          <a href="#platform" className="hover:text-ink transition">
            {c.nav.features}
          </a>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <Link
            to={c.switchTo.href}
            className="hidden sm:inline-flex text-xs font-semibold text-ink/70 hover:text-ink transition px-3 h-8 items-center rounded-full border border-ink/20"
          >
            {c.switchTo.label}
          </Link>
          <Link
          ...
          <Link
            to="/auth"
            className="inline-flex items-center justify-center rounded-full bg-ink text-cream text-sm font-semibold px-5 py-2.5 hover:bg-leaf transition"
          >
            {c.nav.cta}
          </Link>
          <button
            onClick={() => setOpen(!open)}
            className="md:hidden size-10 rounded-full border-2 border-ink flex items-center justify-center"
            aria-label="menu"
          >
            <svg
              width="18"
      ...
      {open && (
        <div className="md:hidden border-t border-ink/10 px-5 py-4 flex flex-col gap-3 text-sm font-medium bg-cream">
          ...
          <Link to={c.switchTo.href} className="text-ink/60">
            {c.switchTo.label}
          </Link>
        </div>
      )}
...
function Sticker({
  ...
  children: React.ReactNode;
  rot?: number;
  tone?: "blush" | "butter" | "leaf" | "cream" | "ink";
  className?: string;
  float?: boolean;
}) {
...
function Stamp({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <span
      className={`inline-flex items-center gap-2 rounded-full border-2 border-ink px-4 h-9 text-[11px] font-bold uppercase tracking-[0.18em] ${className}`}
    >
      {children}
    </span>
  );
...
function SectionHead({
  ...
  return (
    <div className={`mb-12 ${center ? "text-center mx-auto max-w-3xl" : "max-w-3xl"}`}>
      ...
      <h2 className="mt-5 font-display text-4xl sm:text-5xl leading-[1.05] tracking-tight uppercase">
        {title}
      </h2>
  ...
  return (
    <section id="top" className="relative overflow-hidden">
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8 pt-14 sm:pt-20 pb-20 sm:pb-28 text-center">
        ...
        <p className="mt-7 mx-auto max-w-2xl text-base sm:text-lg text-ink/75 leading-[1.7]">
          {c.hero.sub}
        </p>
        <div className="mt-9 flex flex-wrap items-center justify-center gap-3">
          <Pill href="/auth">
            {c.hero.primaryCta} {isRtl ? "←" : "→"}
          </Pill>
          <Pill href="#platform" tone="outline">
            {c.hero.secondaryCta}
          </Pill>
        </div>
        ...
        <div className="relative mt-14 sm:mt-20">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-7">
            <Sticker
              ...
              tone="blush"
              float
              className="p-6 text-start hover:rotate-0 transition-transform duration-300"
            >
            ...
            <Sticker
              ...
              tone="ink"
              float
              className="p-6 text-start lg:mt-8 hover:rotate-0 transition-transform duration-300"
            >
              <div className="text-3xl">🗓️</div>
    ...
    <div className="border-y-2 border-ink bg-ink text-cream py-3 overflow-hidden" dir="ltr">
      ...
      >
        {row.map((w, i) => (
          <span
            key={i}
            className="font-display uppercase text-lg sm:text-xl tracking-wide flex items-center gap-10"
          >
...
function StatCard({ value, label }: { value: string; label: string }) {
  ...
  return (
    <div
      ref={ref}
      className="rounded-2xl border-2 border-ink/85 bg-cream p-6 sm:p-8"
...
function Modules({ c }: { c: C }) {
  ...
  const filtered = useMemo(
    () => c.modules.items.filter((m) => m.tags.includes(activeTab)),
    [c.modules.items, activeTab, activeTab],
  );
  return (
    ...
    <section id="how" className="py-24 sm:py-32 bg-ink text-cream">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <div className="mb-14 text-center mx-auto max-w-3xl">
          <span className="inline-flex items-center rounded-full border-2 border-cream/60 px-4 h-9 text-[11px] font-bold uppercase tracking-[0.18em]">
            {c.how.eyebrow}
          </span>
          <h2 className="mt-5 font-display uppercase text-4xl sm:text-5xl leading-[1.05] tracking-tight">
            {c.how.title}
          </h2>
        </div>
        <div className="grid gap-7 md:grid-cols-3">
          {c.how.steps.map((s, i) => (
            <div
              key={s.n}
              className="rounded-[1.75rem] border-2 border-cream/35 p-8 hover:border-cream/60 transition-colors"
      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        ...
        <div className="grid gap-7 md:grid-cols-3 max-w-6xl mx-auto">
          {c.pricing.plans.map((p, i) => (
            <Sticker
              key={p.name}
              rot={p.featured ? 0 : i === 0 ? -2 : 2}
              tone={p.featured ? "ink" : "cream"}
              className={`p-7 flex flex-col hover:-translate-y-1 transition-transform duration-300 ${p.featured ? "md:-mt-4 md:mb-4" : ""}`}
            >
              ...
              <div className="mt-5 space-y-4 flex-1">
                {(
                  ...
                  <div key={label} className="rounded-2xl border-2 border-current/20 p-4">
                    <div className="text-[0.7rem] font-bold uppercase tracking-[0.14em] opacity-70">
                      {label}
                    </div>
                ...
                {p.bundle && (
                  <div
                    className={`rounded-2xl border-2 border-dashed p-4 ${p.featured ? "border-cream/70" : "border-ink/40"}`}
                  >
                    <div className="text-[0.7rem] font-bold uppercase tracking-[0.14em] opacity-70">
                      {c.pricing.moduleLabels.bundle}
                    </div>
                    <div className="mt-1.5 flex items-baseline gap-2">
                      <div className="font-display text-3xl leading-none">{p.bundle.price}</div>
                      <div className="text-xs opacity-65">{p.bundle.per}</div>
                    </div>
                    <div className="mt-1.5 text-[0.8rem] font-semibold opacity-80">
                      {p.bundle.note}
                    </div>
                  </div>
                )}
        ...
        <p className="mt-10 text-center text-sm font-semibold text-ink/80">
          {c.pricing.ticketFeeNote}
        </p>
      ...
      <div className="mx-auto max-w-3xl px-5 sm:px-8">
        ...
        <div className="space-y-4">
          {c.faq.items.map((item) => (
            <details
              key={item.q}
              className="group rounded-[1.25rem] border-2 border-ink/85 bg-cream px-6 py-5 hover:shadow-[4px_4px_0_0_var(--ink)] transition-shadow"
            >
    ...
    <section className="py-20 sm:py-28">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        <Sticker rot={-1} tone="leaf" className="px-8 sm:px-14 py-16 sm:py-20 text-center">
          <h2 className="font-display uppercase text-4xl sm:text-5xl leading-[1.05] tracking-tight max-w-3xl mx-auto">
            {c.finalCta.title}
          </h2>
          ...
          <div className="mt-9 flex flex-wrap items-center justify-center gap-3">
            <a
              href="/auth"
              className="inline-flex items-center justify-center rounded-full bg-cream text-ink px-7 py-3.5 text-sm font-bold hover:-translate-y-0.5 transition-transform"
            >
              {c.finalCta.primary} {isRtl ? "←" : "→"}
            </a>
            <a
              href="#pricing"
              className="inline-flex items-center justify-center rounded-full border-2 border-cream px-7 py-3.5 text-sm font-bold hover:-translate-y-0.5 transition-transform"
            >
      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-8 py-16">
        <div className="grid gap-10 md:grid-cols-4">
          ...
          {c.footer.cols.map((col) => (
            <div key={col.title}>
              <div className="text-xs font-bold uppercase tracking-[0.18em] text-ink/80">
                {col.title}
              </div>
              <ul className="mt-4 space-y-2.5">
                {col.links.map((l) => (
                  <li key={l}>
                    <a href="#platform" className="text-sm text-ink/65 hover:text-ink transition">
                      {l}
                    </a>
                  </li>
                ))}
        ...
        <div className="mt-12 pt-6 border-t-2 border-ink/15 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-4">
            <Link to={termsHref} className="text-xs text-ink/60 hover:text-ink transition">
              {c.footer.terms}
            </Link>
            <Link to={c.switchTo.href} className="text-xs text-ink/60 hover:text-ink transition">
              {c.switchTo.label}
            </Link>
          </div>
        </div>
