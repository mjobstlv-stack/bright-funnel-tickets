...
import { icons as lucideIcons, Sparkles, Search, Upload, Loader2, X, Check, AlertTriangle } from "lucide-react";
...
import { parseIconValue, serializeIconValue, iconCss, hasIconStyle, type IconStyle } from "@/components/event/icon-style";
...
export function IconValue({ value, className, style }: { value?: string | null; className?: string; style?: React.CSSProperties }) {
  const { icon, style: iconStyle } = parseIconValue(value);
  const css = { ...iconCss(iconStyle), ...style };
  if (/^(https?:|data:image\/)/i.test(icon)) {
    return <img src={icon} alt="" aria-hidden="true" style={css} className={`${className ?? "h-5 w-5"} object-contain`} loading="lazy" />;
  }
  const map = lucideIcons as unknown as Record<string, IconCmp>;
  const Cmp = ((icon && map[icon]) || (Sparkles as unknown as IconCmp)) as React.ComponentType<{ className?: string; style?: React.CSSProperties }>;
  return <Cmp className={className} style={css} aria-hidden="true" />;
}

const POPULAR = [
  "Sparkles", "Music", "PartyPopper", "Wine", "Beer", "UtensilsCrossed", "Pizza", "Coffee",
  "Cake", "IceCream", "Mic", "Disc3", "Camera", "Car", "MapPin", "Clock", "CalendarDays",
  "Users", "Baby", "Dog", "Sun", "Moon", "Flame", "Star", "Heart", "Gift", "Ticket",
  "ShieldCheck", "Accessibility", "Waves", "TreePalm", "Tent", "Trophy", "Wifi", "ParkingCircle",
];
...
export function IconPicker({
  ...
  async function stage(file: File) {
    if (!userId) { toast.error(t("נדרשת התחברות", "Sign in required")); return; }
    const isSvg = file.type === "image/svg+xml" || /\.svg$/i.test(file.name);
    if (!isSvg && !ALLOWED.includes(file.type)) {
      toast.error(t("פורמט לא נתמך — SVG, PNG, WEBP או JPG", "Unsupported format — SVG, PNG, WEBP or JPG"));
      return;
    }
    if (file.size > MAX_BYTES) {
      toast.error(t(`הקובץ ${(file.size / 1024 / 1024).toFixed(2)}MB — מותר עד 1MB`, `File is ${(file.size / 1024 / 1024).toFixed(2)}MB — max 1MB`));
      return;
    }
    if (file.size === 0) { toast.error(t("הקובץ ריק", "File is empty")); return; }
    ...
    try {
      if (isSvg) {
        ...
        if (!/<svg[\s>]/i.test(raw)) { toast.error(t("קובץ ה-SVG פגום", "Invalid SVG file")); return; }
        const sanitized = sanitizeSvg(raw);
        const doc = new DOMParser().parseFromString(sanitized, "image/svg+xml");
        const svg = doc.querySelector("svg");
        if (doc.querySelector("parsererror") || !svg) { toast.error(t("קובץ ה-SVG פגום", "Invalid SVG file")); return; }
        const vb = (svg.getAttribute("viewBox") || "").split(/[\s,]+/).map(Number);
        const w = Number.parseFloat(svg.getAttribute("width") || "") || (vb.length === 4 ? vb[2] : undefined);
        const h = Number.parseFloat(svg.getAttribute("height") || "") || (vb.length === 4 ? vb[3] : undefined);
        const blob = new Blob([sanitized], { type: "image/svg+xml" });
        setPending({ file, previewUrl: URL.createObjectURL(blob), isSvg: true, width: w, height: h, sanitizedSvg: sanitized });
        return;
      }
      ...
      if (!dims) { URL.revokeObjectURL(url); toast.error(t("לא ניתן לקרוא את התמונה", "Could not read the image")); return; }
  ...
  async function confirmUpload() {
    ...
    try {
      ...
      const body: Blob = isSvg && sanitizedSvg ? new Blob([sanitizedSvg], { type: contentType }) : file;
      const path = `${userId}/${scopeId ?? "icons"}/icon-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`;
      const up = await supabase.storage.from("event-media").upload(path, body, { upsert: false, contentType });
      if (up.error) { toast.error(up.error.message); return; }
      const signed = await supabase.storage.from("event-media").createSignedUrl(path, 60 * 60 * 24 * 365 * 10);
      if (signed.error || !signed.data) { toast.error(t("יצירת קישור נכשלה", "Could not create link")); return; }
  ...
  return (
    <Popover open={open} onOpenChange={(o) => { setOpen(o); if (o) setStyle(parseIconValue(value).style); }}>
      ...
      <PopoverContent align="start" className="w-[320px] p-3">
        ...
        <div className="mt-3 grid grid-cols-8 gap-1 max-h-52 overflow-y-auto">
          {results.map((n) => (
            <button
              ...
              type="button"
              title={n}
              onClick={() => { emit(n); setOpen(false); }}
              className={`h-9 w-9 grid place-items-center rounded-md hover:bg-black/5 ${currentIcon === n ? "bg-black/[0.06] ring-1 ring-black/15" : ""}`}
            >
              <IconValue value={n} className="h-4 w-4" style={{ width: undefined, height: undefined, color: style.color }} />
            </button>
          ))}
          {results.length === 0 && (
            <p className="col-span-8 text-xs text-muted-foreground py-3 text-center">{t("לא נמצאו אייקונים", "No icons found")}</p>
          )}
        </div>
        ...
        <div className="mt-3 pt-3 border-t border-black/5 space-y-3">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            {style.color && (
              <Button type="button" variant="ghost" size="sm" className="h-6 px-2 text-[11px]" onClick={() => applyStyle({ ...style, color: undefined })}>
                {t("ברירת מחדל", "Inherit")}
              </Button>
            )}
            <span className="ms-auto grid h-8 w-8 place-items-center rounded-md bg-black/[0.04]">
              <IconValue value={serializeIconValue(currentIcon || "Sparkles", style)} className="h-4 w-4" />
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Slider
              ...
              className="flex-1"
            />
            <span className="text-[11px] tabular-nums w-9 text-end">{(style.size ?? 1).toFixed(2)}×</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Slider
              ...
              className="flex-1"
            />
            <span className="text-[11px] tabular-nums w-9 text-end">{(style.gap ?? 0.28).toFixed(2)}</span>
          </div>
        </div>

        <div className="mt-3 pt-3 border-t border-black/5 space-y-2">
          <p className="text-xs text-muted-foreground">{t("או העלה אייקון משלך (SVG/PNG/WEBP/JPG, עד 1MB)", "Or upload your own icon (SVG/PNG/WEBP/JPG, up to 1MB)")}</p>
          {pending && (
            <div className="rounded-lg border border-black/10 p-2 space-y-2">
              <div className="flex items-center gap-2">
                <span className="h-12 w-12 shrink-0 grid place-items-center rounded-md bg-[repeating-conic-gradient(#0000000d_0_25%,transparent_0_50%)] bg-[length:12px_12px]">
                  <img src={pending.previewUrl} alt="" className="max-h-10 max-w-10 object-contain" />
                </span>
                <div className="min-w-0 text-xs">
                  ...
                  <p className="text-muted-foreground">
                    ...
                    {pending.width && pending.height ? ` · ${Math.round(pending.width)}×${Math.round(pending.height)}` : ""}
                  </p>
                </div>
              </div>
              {pending.isSvg && (
                <p className="flex items-start gap-1 text-[11px] text-muted-foreground">
                  ...
                  {t("ה-SVG נוקה מסקריפטים והפניות חיצוניות", "SVG was sanitized from scripts and external refs")}
                </p>
              )}
              <div className="flex items-center gap-2">
                <Button type="button" size="sm" className="rounded-full" disabled={uploading} onClick={() => void confirmUpload()}>
                  {uploading ? <Loader2 className="h-4 w-4 me-1 animate-spin" /> : <Check className="h-4 w-4 me-1" />}
          ...
          <div className="flex items-center gap-2">
            <Button type="button" variant="outline" size="sm" className="rounded-full" disabled={uploading || checking} onClick={() => fileRef.current?.click()}>
              {checking ? <Loader2 className="h-4 w-4 me-1 animate-spin" /> : <Upload className="h-4 w-4 me-1" />}
          ...
          <input
            ...
            className="hidden"
            onChange={(e) => { const f = e.target.files?.[0]; if (f) void stage(f); e.target.value = ""; }}
          />
        </div>
      </PopoverContent>
    </Popover>
