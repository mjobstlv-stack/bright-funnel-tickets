...
export function mergeSectionsWithDefaults(sections?: SectionEntry[] | null): SectionEntry[] {
  const list = Array.isArray(sections) ? sections.filter((s) => s && typeof s.key === "string") : [];
  const seen = new Set(list.map((s) => s.key));
  const merged: SectionEntry[] = list.map((s) => ({ key: s.key as SectionKey, enabled: s.enabled !== false }));
...
export function EventExtras({
  ...
  if (
    schedule.length === 0 && highlights.length === 0 && !video &&
    includes.length === 0 && faq.length === 0 && rules.length === 0 && !hasLoc && gallery.length === 0
  ) return null;
  ...
  const renderers: Record<SectionKey, () => ReactNode> = {
    highlights: () => highlights.length > 0 && (
        <section aria-labelledby="extras-highlights">
          ...
          <h2 id="extras-highlights" className={`mt-2 ${heading}`}>{L.highlightsTitle}</h2>
          <ul className="mt-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {highlights.slice(0, 6).map((h, i) => (
              <li key={i} className={`rounded-2xl border p-5 text-center ${cardBorder}`}>
                <div className={`mx-auto h-10 w-10 rounded-xl grid place-items-center ${isDark ? "bg-white/10" : "bg-black/[0.04]"}`}>
                  <IconByName name={h.icon} className="h-5 w-5" />
                </div>
                <div className="mt-4 font-medium"><InlineIconText text={h.title} /></div>
                {h.description && <p className={`mt-1 text-sm ${mutedText}`}><InlineIconText text={h.description} /></p>}
              </li>
            ))}
          </ul>
        </section>
    ...
    includes: () => includes.length > 0 && (
        <section aria-labelledby="extras-includes">
          ...
          <h2 id="extras-includes" className={`mt-2 ${heading}`}>{L.includesTitle}</h2>
          <ul className="mt-6 grid sm:grid-cols-2 gap-2">
            {includes.map((it, i) => (
              <li key={i} className={`flex items-start gap-3 rounded-xl border p-4 ${cardBorder}`}>
                <div className={`h-8 w-8 shrink-0 rounded-lg grid place-items-center ${isDark ? "bg-white/10" : "bg-black/[0.04]"}`}>
    ...
    location: () => hasLoc && (
        <section aria-labelledby="extras-location">
          ...
          <h2 id="extras-location" className={`mt-2 ${heading}`}>{L.locationTitle}</h2>
          <div className={`mt-6 rounded-2xl border overflow-hidden ${cardBorder}`}>
                ...
                {(gmapsHref || wazeHref) && (
                  <div className="absolute top-3 start-3 flex flex-wrap gap-2 z-10">
                    {gmapsHref && (
                      ...
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-3.5 w-3.5"><path d="M15 3h6v6"/><path d="M10 14 21 3"/><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/></svg>
                        {L.maps}
                      </a>
                    )}
                    {wazeHref && (
            ...
            {!mapEmbedSrc && (gmapsHref || wazeHref) && (
              <div className="flex flex-wrap gap-2 p-4">
                {wazeHref && (
                  <a href={wazeHref} target="_blank" rel="noopener noreferrer" className={`inline-flex items-center gap-1.5 text-sm rounded-full px-4 py-2 border ${isDark ? "border-white/20 hover:bg-white/10" : "border-black/15 hover:bg-black/[0.04]"}`}>
                    Waze →
                  </a>
                )}
                {gmapsHref && (
                  <a href={gmapsHref} target="_blank" rel="noopener noreferrer" className={`inline-flex items-center gap-1.5 text-sm rounded-full px-4 py-2 border ${isDark ? "border-white/20 hover:bg-white/10" : "border-black/15 hover:bg-black/[0.04]"}`}>
                    Google Maps →
                  </a>
                )}
              </div>
            ...
            {loc?.address && (
              <div>
                <div className={`text-[11px] uppercase tracking-widest ${mutedText}`}>{L.address}</div>
                <p className="mt-1 text-sm leading-relaxed">{loc.address}</p>
              </div>
            )}
            {loc?.parking && (
              <div>
                <div className={`text-[11px] uppercase tracking-widest ${mutedText}`}>{L.parking}</div>
                <p className="mt-1 text-sm leading-relaxed">{loc.parking}</p>
              </div>
            )}
            {loc?.transit && (
              <div>
                <div className={`text-[11px] uppercase tracking-widest ${mutedText}`}>{L.transit}</div>
                <p className="mt-1 text-sm leading-relaxed">{loc.transit}</p>
              </div>
            )}
            {loc?.notes && (
              <div>
                <div className={`text-[11px] uppercase tracking-widest ${mutedText}`}>{L.notes}</div>
                <p className="mt-1 text-sm leading-relaxed">{loc.notes}</p>
              </div>
            )}
            {loc?.map_url && !mapQuery && (
              <div className="sm:col-span-2">
                <a href={loc.map_url} target="_blank" rel="noopener noreferrer" className={`inline-flex items-center gap-1.5 text-sm underline underline-offset-4 ${isDark ? "text-white" : "text-foreground"}`}>
                  {L.openMaps}
                </a>
              </div>
            )}
    ...
    schedule: () => schedule.length > 0 && (
        <section aria-labelledby="extras-schedule">
          ...
          <h2 id="extras-schedule" className={`mt-2 ${heading}`}>{L.scheduleTitle}</h2>
          <ol className={`mt-6 rounded-2xl border divide-y ${isDark ? "border-white/15 divide-white/10 bg-white/5" : "border-black/10 divide-black/5 bg-white"}`}>
            {schedule.map((s, i) => (
              <li key={i} className="p-4 sm:p-5 grid grid-cols-[88px_1fr] sm:grid-cols-[120px_1fr] gap-4 items-start">
                <div className={`text-sm font-mono ${mutedText}`}>{s.time || "—"}</div>
                <div className="min-w-0">
                  <div className="font-medium"><InlineIconText text={s.title} /></div>
                  {s.description && <p className={`mt-0.5 text-sm ${mutedText}`}><InlineIconText text={s.description} /></p>}
                </div>
              </li>
            ))}
          </ol>
    ...
    faq: () => faq.length > 0 && (
        <section aria-labelledby="extras-faq">
          ...
          <h2 id="extras-faq" className={`mt-2 ${heading}`}>{L.faqTitle}</h2>
          <div className={`mt-6 rounded-2xl border divide-y ${isDark ? "border-white/15 divide-white/10 bg-white/5" : "border-black/10 divide-black/5 bg-white"}`}>
            {faq.map((f, i) => (
              <details key={i} className="group p-4 sm:p-5 [&_summary::-webkit-details-marker]:hidden">
                <summary className="flex items-start justify-between gap-4 cursor-pointer list-none">
                  <span className="font-medium"><InlineIconText text={f.question} /></span>
                  <span className={`mt-0.5 text-lg leading-none transition-transform group-open:rotate-45 ${mutedText}`}>+</span>
                </summary>
                <p className={`mt-3 text-sm leading-relaxed whitespace-pre-line ${mutedText}`}><InlineIconText text={f.answer} /></p>
              </details>
            ))}
          </div>
        </section>
    ...
    rules: () => rules.length > 0 && (
        <section aria-labelledby="extras-rules">
          ...
          <h2 id="extras-rules" className={`mt-2 ${heading}`}>{L.rulesTitle}</h2>
          <ul className={`mt-6 rounded-2xl border divide-y ${isDark ? "border-white/15 divide-white/10 bg-white/5" : "border-black/10 divide-black/5 bg-white"}`}>
            {rules.map((r, i) => (
              <li key={i} className="p-4 sm:p-5 flex items-start gap-3">
                <span className={`mt-1.5 h-1.5 w-1.5 rounded-full shrink-0 ${isDark ? "bg-white/70" : "bg-black/60"}`} />
    ...
    video: () => video && (
        <section aria-labelledby="extras-video">
          ...
          <h2 id="extras-video" className={`mt-2 ${heading}`}>{L.videoTitle}</h2>
          <div className={`mt-6 aspect-video rounded-2xl overflow-hidden border ${isDark ? "border-white/15" : "border-black/10"} bg-black`}>
            {video.kind === "iframe" ? (
              <iframe
                src={video.src}
                title="Event video"
              ...
              <video src={video.src} controls playsInline className="h-full w-full object-contain" />
            )}
          </div>
        </section>
      ),
    gallery: () => gallery.length > 0 && (
        <section aria-labelledby="extras-gallery">
          ...
          <h2 id="extras-gallery" className={`mt-2 ${heading}`}>{L.galleryTitle}</h2>
          <GallerySlider images={gallery} isDark={isDark} rtl={he} labels={{ prev: L.prev, next: L.next }} />
        </section>
      ),
  };
...
function GallerySlider({
  ...
  const btn = `h-10 w-10 rounded-full grid place-items-center border shadow-sm transition-colors ${
    isDark ? "border-white/20 bg-black/50 text-white hover:bg-black/70" : "border-black/10 bg-white/90 text-foreground hover:bg-white"
  }`;
    ...
    <div className="relative mt-6 min-w-0">
      ...
      {images.length > 1 && (
        <div className="mt-3 flex items-center justify-end gap-2">
          <button type="button" aria-label={labels.prev} className={btn} onClick={() => scrollBy(-1)}>
            {rtl ? <ChevronRight className="h-4 w-4" /> : <ChevronLeft className="h-4 w-4" />}
          </button>
          <button type="button" aria-label={labels.next} className={btn} onClick={() => scrollBy(1)}>
