...
import { Sparkles, Music, GraduationCap, Tent, Upload, X, Loader2, Check, PenLine, Image as ImageIcon, FileText } from "lucide-react";

type Category = "concert" | "conference" | "festival";

function slugify(s: string) {
  return s.toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || `event-${Math.random().toString(36).slice(2, 6)}`;
}
...
export function EventGeneratorDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  const { user } = useAuth();
  ...
  const categories = useMemo(() => ([
    ...
    { id: "conference" as const, icon: GraduationCap, he: "כנס / וובינר", en: "Conference / Webinar" },
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (!org) throw new Error(t("לא נמצא ארגון — סיימי הרשמה קודם", "No organization — complete onboarding first"));

      // 3. Create draft event (placeholder name; will update after AI)
      const tempName = prompt.split(/[.\n!?]/)[0].slice(0, 60) || (lang === "he" ? "אירוע חדש" : "New event");
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        {phase === "form" && (<>
          <DialogHeader>
            ...
            <DialogDescription>
              {t("3 שלבים קצרים ואת מקבלת דף מוכן. אפשר לערוך הכל אחר כך.", "3 quick steps and you get a ready page. Edit anything after.")}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6 py-2">
            ...
            {/* Step 2 */}
            <section>
              <Label htmlFor="gen-prompt" className="text-sm font-medium">{t("2. ספרי לי על האירוע", "2. Tell me about the event")}</Label>
              <Textarea
                id="gen-prompt"
                rows={4}
                value={prompt}
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {prompt.trim().length}/3000 · {t("כמה שיותר פרטים = תוצאה טובה יותר", "More detail = better result")}
              </p>
            </section>
            ...
            <section>
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium">{t("3. הזהות שלך", "3. Your identity")}</Label>
                <span className="text-xs text-muted-foreground">{t("רשות — אבל מומלץ", "optional but recommended")}</span>
              </div>
              ...
              <div className="mt-3 grid gap-2">
                <UrlInput value={website} onChange={setWebsite} placeholder={t("אתר קיים (https://...)", "Existing website (https://...)")} />
                <UrlInput value={instagram} onChange={setInstagram} placeholder="Instagram (@handle or URL)" />
          ...
          <DialogFooter className="gap-2">
            <Button variant="ghost" onClick={close}>{t("ביטול", "Cancel")}</Button>
            <Button
        ...
        {phase === "generating" && (
          <div className="py-16 flex flex-col items-center justify-center gap-4 text-center">
            ...
            <div>
              <div className="font-display text-xl">{t("בונה את הדף שלך…", "Building your page…")}</div>
              <p className="text-sm text-muted-foreground mt-1">
                {t("קורא את הלוגו, בוחר צבעים, כותב טקסטים, בונה סקשיינים.", "Reading logo, picking colors, writing copy, building sections.")}
              </p>
        ...
        {phase === "done" && (
          <div className="py-8 text-center">
            ...
            <h2 className="mt-4 font-display text-2xl">{t("הדף מוכן ✨", "Your page is ready ✨")}</h2>
            <p className="mt-2 text-sm text-muted-foreground max-w-md mx-auto">
              {t("איך את מרגישה עם זה? אפשר לפרסם עכשיו, או להוסיף טאץ' אישי.", "How does it feel? You can publish now, or add a personal touch.")}
            </p>
...
function FileSlot({ icon: Icon, label, hint, file, onPick, onClear }: {
  ...
  return (
    ...
    >
      <div className={`grid place-items-center h-9 w-9 rounded-lg ${file ? "bg-emerald-100 text-emerald-700" : "bg-black/5"}`}>
